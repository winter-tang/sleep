  ...
  useEffect(() => {
    ...
    window.playAlarm = function() {
        ...
        try {
          ...
          if (isAndroid.current && window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.AlarmAudioBridge) {
            log.info('使用Android原生闹钟播放');
            
            // 尝试获取闹钟音频路径
            let alarmPath = null;
            if (alarmRef.current && alarmRef.current.src) {
              alarmPath = alarmRef.current.src;
            }
            
            // 调用原生方法播放闹钟
            window.Capacitor.Plugins.AlarmAudioBridge.playAlarm({
              audioPath: alarmPath,
              ...
              log.error('Android原生闹钟播放出错:', error);
              log.warn('回退到HTML5播放');
              playHTML5Alarm();
            });
          } else {
