...
describe('LogManager', () => {
  ...
  describe('log', () => {
    ...
    it('应该添加error级别的日志', () => {
      const message = '测试错误日志'
      const error = new Error('测试错误')
      logManager.log(message, 'error', error)
  ...
  describe('便捷方法', () => {
    ...
    it('error方法应该添加error级别的日志', () => {
      const message = '测试error方法'
      const error = new Error('测试错误')
      logManager.error(message, error)
      
      const logs = logManager.getLogs()
      expect(logs).toHaveLength(1)
      expect(logs[0].level).toBe('error')
      expect(logs[0].message).toBe(message)
      expect(logs[0].error).toBe(error.toString())
    })
